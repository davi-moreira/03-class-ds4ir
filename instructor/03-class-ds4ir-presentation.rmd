...
congress_us %>% 
  group_by(congress, party) %>%
  summarise(dwnom2 = median(dwnom2)) %>%
  filter(party %in% c("Democrat", "Republican")) %>%
  ggplot(aes(x = congress, y = dwnom2,
             colour = fct_reorder2(party, congress, dwnom2))) +
  geom_line() +
  ylim(-1,1) +
  labs(y = "DW-NOMINATE score (2st Dimension)", x = "Congress",
       colour = "Party")
```
--->
...
```{r message=FALSE, warning=FALSE}
# solução
gini_congress <- gini_us %>% select(gini) %>% slice(seq(2, nrow(gini_us), by = 2))

# alternativa
alternativa <- gini_us$gini[seq(2, nrow(gini_us), by = 2)]

# validação
gini_congress$gini %in% alternativa
sum(gini_congress$gini %in% alternativa) == length(gini_congress$gini)
```
